...
There are quite many configs in Cadence. Here are the most basic configs that you should understand. The sample configs can be found in `cadence/config/development.yaml`.
...
* Local docker-compose by logging into the container: 1. Run `docker exec -it docker_cadence_1 /bin/bash` to login your container. 2. Then `vi config/dynamicconfig/development.yaml` to make any change. 3. After you changed the config, use `docker restart docker_cadence_1` to restart the cadence instance. Note that you can also use this approach to change static config, but it must be changed through `config/config_template.yaml` instead of `config/docker.yaml` because `config/docker.yaml` is generated on startup.
...
After changing the client to `configstore` and restarting Cadence, you can manage dynamic configs using `cadence admin config` CLI commands.
